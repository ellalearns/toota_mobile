...
import signIn from "@/services/signindriver";
import signUp from "@/services/signupdriver";
...
const Auth = () => {
    ...
    const createAccount = async () => {
        ...
        const DriverNow = await signUp({ data: newDriver })

        if (DriverNow instanceof Error) {
            ToastAndroid.show(DriverNow.message, 1000)
        } else {
            router.push({
    ...
    const logIn = async () => {
        ...
        const driverLogIn = await signIn({ data: driver })

        if (driverLogIn instanceof Error) {
